...
const handleShow = paket => {
    if (!paket?.id) return
    router.visit(route('admin.rendan.show', { rendan: paket.id }))
}
...
const columns = [
    ...
    columnHelper.display({
        ...
        cell: info => {
            ...
            const editButton = paket.enjiniring?.rendan
                ? h(
                      'button',
                      {
                          class: 'p-2 text-blue-400 hover:text-blue-600 transition-colors rounded-lg hover:bg-blue-50 cursor-pointer',
                          onClick: () => handleEdit(paket.enjiniring.rendan), // Ganti handleEdit
                          type: 'button',
                          title: 'Edit'
                      },
                      [
            ...
            const deleteButton = paket.enjiniring?.rendan
                ? h(
                      'button',
                      {
                          class: 'p-2 text-red-400 hover:text-red-600 transition-colors rounded-lg hover:bg-red-50 cursor-pointer',
                          onClick: () => confirmDeleteRendan(paket.enjiniring.rendan), // Pastikan nama fungsi sesuai logic delete Anda
                          type: 'button',
                          title: 'Hapus'
                      },
                      [
